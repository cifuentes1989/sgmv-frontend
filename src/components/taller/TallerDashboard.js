...
const TallerDashboard = () => {
  ...
  // --- NUEVA FUNCIÓN: TEXTO RÁPIDO PARA REDUCIR ESCRITURA EN CELULAR ---
  ...
  // 4. MANEJADORES DE FORMULARIOS (INTACTOS)
  ...
  // --- FUNCIÓN DE COLORES (Como en el conductor) ---
    ...
    <main style={{ width: '100%', maxWidth: '800px', margin: '0 auto', padding: '15px', backgroundColor: '#f4f7f6', minHeight: '100vh', boxSizing: 'border-box' }}>
      ...
      <details open style={{ marginBottom: '20px', backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <summary style={{ padding: '15px', backgroundColor: '#fff3e0', color: '#e65100', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer', outline: 'none' }}>
            🛠️ Pendientes de Diagnóstico ({pendientes.length})
        </summary>
        <div style={{ padding: '15px' }}>
            {pendientes.length > 0 ? pendientes.map((solicitud) => (
            <div key={solicitud.id} style={{ borderBottom: '1px solid #eee', paddingBottom: '15px', marginBottom: '15px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                    <strong style={{ fontSize: '1.1rem' }}>{solicitud.nombre_vehiculo} ({solicitud.placa_vehiculo})</strong>
                    <span style={{ fontSize: '0.8rem', color: '#888' }}>ID #{solicitud.id}</span>
                </div>
                
                <div style={{ backgroundColor: '#f9f9f9', padding: '10px', borderRadius: '8px', fontSize: '0.9rem', marginBottom: '15px' }}>
                    <p style={{ margin: '0 0 5px 0' }}><strong>Conductor:</strong> {solicitud.nombre_conductor}</p>
                    <p style={{ margin: '0', color: '#d32f2f' }}><strong>⚠️ Falla Reportada:</strong> {solicitud.necesidad_reportada}</p>
                </div>
      ...
      <details open style={{ marginBottom: '20px', backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <summary style={{ padding: '15px', backgroundColor: '#e3f2fd', color: '#1565c0', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer', outline: 'none' }}>
            ⚙️ En Reparación ({enReparacion.length})
        </summary>
        <div style={{ padding: '15px' }}>
      ...
      <details style={{ marginBottom: '20px', backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
        <summary style={{ padding: '15px', backgroundColor: '#eeeeee', color: '#424242', fontWeight: 'bold', fontSize: '1.1rem', cursor: 'pointer', outline: 'none' }}>
            📚 Historial de Trabajos ({historial.length})
        </summary>
        <div style={{ padding: '15px' }}>
            {historial.length > 0 ? historial.map((s) => {
                ...
                <div key={s.id} style={{ borderBottom: '1px solid #eee', paddingBottom: '15px', marginBottom: '15px' }}>
                    ...
                    <details style={{ outline: 'none' }}>
                        ...
                        <div style={{ padding: '12px', backgroundColor: '#fafafa', borderRadius: '8px', marginTop: '10px', fontSize: '0.85rem', color: '#555', borderLeft: '3px solid #0288d1' }}>
                            <p style={{ margin: '0 0 5px 0' }}><strong>1. Falla:</strong> {s.necesidad_reportada}</p>
                            <p style={{ margin: '0 0 5px 0' }}><strong>2. Diagnóstico:</strong> {s.diagnostico_taller}</p>
                            <p style={{ margin: '0 0 5px 0' }}><strong>3. Trabajo Realizado:</strong> {s.trabajos_realizados}</p>
                            <p style={{ margin: '0 0 5px 0' }}><strong>Repuestos:</strong> {s.repuestos_utilizados || 'Ninguno'}</p>
                            {s.motivo_rechazo && <p style={{ margin: '5px 0 0 0', color: '#d32f2f' }}><strong>Motivo Rechazo:</strong> {s.motivo_rechazo}</p>}
                        </div>
                    </details>
                </div>
